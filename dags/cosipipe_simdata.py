...
def stage_all_files_external(
    *,
    ...
    """
    Executed in cosipy env. Avoids re-downloading/re-extracting files that are already ready.
    Detects corrupted .gz/.zip files (redownloads) and validates the background FITS if already present.
    """
    ...
    def validate_background_fits(f: Path) -> bool:
        ...
        try:
            ...
            with fits.open(f, memmap=True) as hdul:
                # minimal access to force reading of the table block
    ...
    def download_or_use_one(remote_or_local: str, target_dir: Path, *, kind: str) -> list[Path]:
        ...
        # If ready files exist, validate the FITS first for BACKGROUND
        if ready_files_exist(target_dir):
            files = ready_files(target_dir)
            if kind == "background":
                # validate the first FITS file
...
def run_bkg_cut_and_move(*, script: str, products_dir: str, source_link_path: str, background_link_path: str, eps_time: float):
    """
    Runs bkg_cut in the products folder using SYMLINKs as input.
    Does not move anything: the script must generate output directly in products/.
    Returns the list of new files created in products.
    """
...
with DAG(
    ...
    def prepare_raw_dirs():
        # Ensure that all raw data subdirectories exist
    ...
    def create_symlinks(ti):
        """
        Creates symlinks in products/ for:
        - source file
        - response file(s) (can be more than one after unzip)
        - orientation file
        - (optional) original background
        The 'cut' background file(s) are already in products/ from the previous task,
        so no symlinks are needed for those.
        """
        staged = ti.xcom_pull(task_ids="stage_all_files")
        products_dir = Path(ti.xcom_pull(task_ids="create_products_dir")["products_dir"])

        # If multiple files exist (e.g. response after unzip), link them all
        files_by_kind = {
