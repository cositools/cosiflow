...
def build_custom(dag):
    """
    Tutorial A:
    build a binary text matrix, factorize via SVD, save A/B and plots.
    """

    # -------------------------------------------------
    # 0) Declare the result folder for COSIDAG
    # -------------------------------------------------
    ...
    # -------------------------------------------------
    # 1) Factorize text matrix (external env)
    # -------------------------------------------------
    ...
    a_factorize = ExternalPythonOperator(
        ...
        python_callable=_a_make_factors,   # IDENTICA al tutorial originale
        op_kwargs={
            "base_dir": BASE_DIR,
            "text": TEXT,
            "size": SIZE,
    ...
    set_detected_folder >> a_factorize


with COSIDAG(
    dag_id="cosidag_tutorial_a_svd",
    ...
    tags=["cosidag", "tutorial", "external-python"],
) as dag:
    pass
